...
public class StepClimber : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        bool grounded = FindGround(out groundCP, this.allCPs);

        Vector3 stepUpOffset = default(Vector3);
        bool stepUp = false;
        if (grounded)
        {
            stepUp = FindStep(out stepUpOffset, this.allCPs, groundCP, velocity);
        }
        ...
        if (stepUp)
        {
            ...
            this.GetComponent<Rigidbody>().velocity = this.lastVelocity;
        }
    ...
    {
        this.allCPs.AddRange(col.contacts);
    }
    ...
    bool ResolveStepUp(out Vector3 stepUpOffset, ContactPoint stepTestCP, ContactPoint groundCP)
    {
        ...
        if (!(stepTestCP.point.y - groundCP.point.y < this.maxStepHeight))
        {
            return false;
        }
        ...
        //Fires one Raycast
        RaycastHit hitInfo;
        float stepHeight = groundCP.point.y + this.maxStepHeight + 0.0001f;
        Vector3 stepTestInvDir = new Vector3(-stepTestCP.normal.x, 0, -stepTestCP.normal.z).normalized;
        Vector3 origin = new Vector3(stepTestCP.point.x, stepHeight, stepTestCP.point.z) + (stepTestInvDir * this.stepSearchOvershoot);
        Vector3 direction = Vector3.down;
        if (!(stepCol.Raycast(new Ray(origin, direction), out hitInfo, this.maxStepHeight)))
        {
            return false;
        }

        //We have enough info to calculate the points
        Vector3 stepUpPoint = new Vector3(stepTestCP.point.x, hitInfo.point.y + 0.0001f, stepTestCP.point.z) + (stepTestInvDir * this.stepSearchOvershoot);
